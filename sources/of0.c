...
{
if (parent1->dodag->new_p==0 || parent2->dodag->new_p==0){    
...
else if (parent1->dodag->new_p==1 || parent2->dodag->new_p==1){

        if (parent1->link_metric < parent2->link_metric && parent1->link_metric!=MET_BASE) {
            return 1;
        }
        else if (parent1->link_metric > parent2->link_metric) {
            return -1;
        }
    }
return 0;
