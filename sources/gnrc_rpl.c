...
static void _receive(gnrc_pktsnip_t *icmpv6)
{
    ...
    switch (icmpv6_hdr->code) {
        ...
        case GNRC_RPL_ICMPV6_CODE_DAO:
            ...
            int lock = 0;
            for(int i=0;i<10;i++){
                if(ipv6_addr_equal(&gnrc_rpl_instances[0].dodag.childs[i],&ipv6_hdr->src)){lock=1;}}
                if(lock==0){
                    gnrc_rpl_instances[0].dodag.childs[gnrc_rpl_instances[0].dodag.nb_childs]=ipv6_hdr->src;
